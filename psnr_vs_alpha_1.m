...
for i = 1:length(alpha_values)
    ...
    % result = [];
    % for i=1:20
    %     C_clip_r = i * C_clip_r; % Example: doubling the clipping limit for Red channel
    %     C_clip_g = i * C_clip_g; % Example: doubling the clipping limit for Green channel
    %     C_clip_b = i * C_clip_b; % Example: doubling the clipping limit for Blue channel
    %     ssim_value = ssim(image_rgb,image_out);
    %     result = [ssim_value result];
    % end
    % [best_result i]= max(ssid)
    
    % Increase the clipping limit for more enhancement
    C_clip_r = 1 * C_clip_r; % Example: doubling the clipping limit for Red channel
    C_clip_g = 1 * C_clip_g; % Example: doubling the clipping limit for Green channel
    C_clip_b = 1 * C_clip_b; % Example: doubling the clipping limit for Blue channel
    ...
    % Decrease the standard deviation by half
    SD_r = SD_r / 1;
    SD_g = SD_g / 1;
    SD_b = SD_b / 1;
    ...
    % Increase the partition range by adding a fixed value
    partition_range = 0; % Example value, you can adjust it as per your requirement
    L_low_r = L_low_r + partition_range;
    L_high_r = L_high_r - partition_range;
    L_low_g = L_low_g + partition_range;
    L_high_g = L_high_g - partition_range;
    L_low_b = L_low_b + partition_range;
    L_high_b = L_high_b - partition_range;
    ...
    %
    % % Transfer Function Generation for each sub-energy curve for Red channel
    % T_L_r= L_low_r*cdf_L_r;
    % T_M_r= (L_low_r +1) + (L_high_r - L_low_r +1)*cdf_M_r;
    % T_U_r= (L_high_r +1) + (L - L_high_r +1)*cdf_U_r;
    % %Final Transfer Function for Red channel
    % T_r= [T_L_r, T_M_r, T_U_r];
    % %Transfer Function Generation for each sub-energy curve for Green channel
    % T_L_g= L_low_g*cdf_L_g;
    % T_M_g= (L_low_g +1) + (L_high_g - L_low_g +1)*cdf_M_g;
    % T_U_g= (L_high_g +1) + (L - L_high_g +1)*cdf_U_g;
    % %Final Transfer Function for Green channel
    % T_g= [T_L_g, T_M_g, T_U_g];
    % % Transfer Function Generation for each sub-energy curve for Blue channel
    % T_L_b= L_low_b*cdf_L_b;
    % T_M_b= (L_low_b +1) + (L_high_b - L_low_b +1)*cdf_M_b;
    % T_U_b= (L_high_b +1) + (L - L_high_b +1)*cdf_U_b;
    % %Final Transfer Function for Blue channel
    % T_b= [T_L_b, T_M_b, T_U_b];
    
    % Adjust the transfer functions
    % alpha = 0.8 ; % Adjustment parameter (increase or decrease as desired)
